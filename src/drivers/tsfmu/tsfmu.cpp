...
private:
	...
	static void fit_curve_trampoline(void *arg);
	void		fit_curve();
...
void
TSFMU::fit_curve_trampoline(void * arg){
	TSFMU *dev = reinterpret_cast<TSFMU *>(arg);
...
void
TSFMU::calc_pid_param(){
	orb_advert_t handle;
	actuator_armed_s aa;
	if(_pwm_limit.state != PWM_LIMIT_STATE_ON){
		aa.armed = true;
		aa.prearmed = true;
		aa.ready_to_arm = true;
		aa.lockdown = false;
	...
	int hit_check_point[2] = {0,2};
	int hit_check_point1[2] = {0,2};
...
//							   start[1] + jump_step * 0.90f};
	...
	while(hit_check_point[0] != 2 || hit_check_point1[0] != 2 || hit_check_point[1] != 2 || hit_check_point1[1] != 2){
		pret = px4_poll(&fds[0], (sizeof(fds) / sizeof(fds[0])), 0.1);
	...
	printf("current voltage %.6f\n", (double)( _curr_batt_volt / 1000.0f));
	usleep(2000000);
...
{	perf_count(_cycle_exec);
	...
	if (!_initialized) {
		...
		_batt_status_sub = orb_subscribe(ORB_ID(battery_status));
...
//			}
			...
			int pret = px4_poll(&fds[0], (sizeof(fds) / sizeof(fds[0])), 0);
	...
	orb_check(_batt_status_sub, &updated);
	if (updated) {
		orb_copy(ORB_ID(battery_status), _batt_status_sub, &_batt_status_msg);
		memcpy(&_curr_batt_volt, &_batt_status_msg.voltage_filtered_v, sizeof(_batt_status_msg.voltage_filtered_v));
	}
...
void TSFMU::work_stop()
{
	...
	work_cancel(HPWORK, &_work_calib);
	...
	orb_unsubscribe(_armed_sub);
	orb_unsubscribe(_param_sub);
	orb_unsubscribe(_batt_status_sub);
	orb_unsubscribe(_esc_rads_sub);
