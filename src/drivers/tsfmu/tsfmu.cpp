...
//	for(int j=0; j<CALIB_NUM_INTVL; j++){
//		A_l[j][0] = j*j;
//		A_l[j][1] = j;
//		A_l[j][2] = 1;
//		y_l[j] = j*j + 2 * j + 3;
...
//	}
	...
	printf("curr voltage: %.5f",(double) _curr_batt_volt);
...
//	} // poll_fds
	...
	if (updated) {
		orb_copy(ORB_ID(actuator_armed), _batt_status_sub, &_batt_status_msg);
		_curr_batt_volt = _batt_status_msg.voltage_v;
	}
